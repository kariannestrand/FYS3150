...
public:
    ...
    // constructor
    PenningTrap(double B0, double V0, double d, double ke, double f, double omega_v, int n, double N, arma::mat pos, arma::mat vel, arma::vec q_vec, arma::vec m_vec, bool write, bool interaction, bool modified);

    // class methods to be made
    arma::vec external_E_field(int i, double dt);       // external electric field at point r=(x,y,z)
    ...
    arma::vec total_force_external(int i, double dt);   // the total force on particle_i from the external fields
    arma::vec total_force_particles(int i);             // the total force on particle_i from the other particles
    arma::vec total_force(int i, double dt);            // the total force on particle_i from both external fields and other particles
